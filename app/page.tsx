...
const navItems = [['⌂','Início'],['◔','Resumo'],['▦','Parcelas'],['●','Cadastros']] as const;
...
export default function Home() {
  ...
  const [editingCategory,setEditingCategory]=useState(''); const [categoryName,setCategoryName]=useState(''); const [categoryKind,setCategoryKind]=useState<'FIXO'|'VARIAVEL'>('VARIAVEL');
  ...
  const [catalogSection,setCatalogSection]=useState<'Categorias'|'Pessoas'|'Origens'>('Categorias'); const [selectedInstallmentMonth,setSelectedInstallmentMonth]=useState(today().slice(0,7));
  ...
  async function saveCategory(event:FormEvent){
    ...
    const values={name:categoryName.trim(),kind:categoryKind,icon:categoryKind==='FIXO'?'home':'category',color_hex:categoryKind==='FIXO'?'#8250DF':'#1A365D'};
    ...
    if(result.error)setNotice(catalogError(result.error));else{setCategoryName('');setEditingCategory('');setCategoryKind('VARIAVEL');setNotice(editingCategory?'Categoria atualizada.':'Categoria adicionada.');await loadWorkspace(user);}setBusy(false);
  }
  ...
  const installmentExpenses=useMemo(()=>expenses.filter(item=>item.installments>1),[expenses]);
  ...
  {activeTab==='Parcelas'&&<section className="installments-page"><section className="panel installment-timeline"><div className="panel-title"><div><p className="eyebrow">PASSADO E FUTURO</p><h2>Total de parcelas por mês</h2></div><span className="timeline-help">Toque em um mês para abrir</span></div>{installmentMonths.length?<div className="month-strip">{installmentMonths.map(item=>{const position=Math.max(12,Math.round(item.total/maxInstallmentMonth*58));const selected=item.key===selectedInstallmentMonth;const period=item.key<currentMonth?'past':item.key===currentMonth?'current':'future';return <button type="button" key={item.key} className={`month-card ${selected?'selected':''} ${period}`} onClick={()=>setSelectedInstallmentMonth(item.key)}><strong>{new Date(`${item.key}-02T12:00:00`).toLocaleDateString('pt-BR',{month:'short'}).replace('.','')}</strong><span className="month-year">{item.key.slice(0,4)}</span><span className="month-chart"><i style={{height:`${position}px`}} /><b /></span><em>{money(item.total)}</em></button>})}</div>:<p className="empty-copy">Nenhuma compra parcelada encontrada.</p>}</section><section className="installment-summary"><article><small>Total em {selectedMonthLabel}</small><strong>{money(selectedInstallmentTotal)}</strong></article><article><small>Parcelas no mês</small><strong>{selectedInstallments.length}</strong></article><article><small>Depois deste mês</small><strong>{money(futureInstallmentTotal)}</strong></article></section><section className="panel installment-details"><div className="panel-title"><div><p className="eyebrow">DETALHAMENTO</p><h2>Parcelas de {selectedMonthLabel}</h2></div></div><div className="installment-list">{selectedInstallments.map(item=><article key={item.id}><span className="installment-number">{item.installment}<small>de {item.installments}</small></span><div><strong>{item.title}</strong><small>{item.category} · {item.person} · {item.origin}</small></div><time>{new Date(item.date+'T12:00:00').toLocaleDateString('pt-BR')}</time><b>{money(item.value)}</b></article>)}{!selectedInstallments.length&&<p className="empty-copy">Não há parcelas neste mês.</p>}</div></section></section>}
  {activeTab==='Cadastros'&&<div className="catalog-tabs" role="tablist" aria-label="Tipos de cadastro">{(['Categorias','Pessoas','Origens'] as const).map(item=><button type="button" role="tab" aria-selected={catalogSection===item} className={catalogSection===item?'active':''} key={item} onClick={()=>setCatalogSection(item)}>{item}</button>)}</div>}
  {activeTab==='Cadastros'&&catalogSection==='Categorias'&&<section className="catalog-layout"><form className="panel catalog-form" onSubmit={saveCategory}><div className="panel-title"><div><p className="eyebrow">{editingCategory?'EDITAR':'NOVA'} CATEGORIA</p><h2>{editingCategory?'Atualizar categoria':'Adicionar categoria'}</h2></div></div><label><span>Nome</span><input value={categoryName} onChange={e=>setCategoryName(e.target.value)} placeholder="Ex.: Educação" required /></label><label><span>Tipo</span><select value={categoryKind} onChange={e=>setCategoryKind(e.target.value as 'FIXO'|'VARIAVEL')}><option value="VARIAVEL">Variável</option><option value="FIXO">Fixo</option></select></label><div className="catalog-form-actions">{editingCategory&&<button type="button" className="attach" onClick={()=>{setEditingCategory('');setCategoryName('');setCategoryKind('VARIAVEL')}}>Cancelar</button>}<button className="save" disabled={busy}>{editingCategory?'Salvar alterações':'Adicionar'}</button></div>{notice&&<p className="form-notice" role="status">{notice}</p>}</form><section className="panel catalog-panel"><div className="panel-title"><div><p className="eyebrow">ORGANIZAÇÃO</p><h2>Suas categorias</h2></div><span className="count-badge">{categories.length}</span></div><div className="catalog-list">{categories.map(item=><article key={item.id}><span className="catalog-icon">{item.kind==='FIXO'?'⌂':'●'}</span><div><strong>{item.name}</strong><small>{item.kind==='FIXO'?'Custo fixo':'Custo variável'}</small></div><div className="row-actions"><button type="button" onClick={()=>{setEditingCategory(item.id);setCategoryName(item.name);setCategoryKind(item.kind)}}>Editar</button><button type="button" className="danger" onClick={()=>removeCatalog('categories',item.id,item.name)}>Excluir</button></div></article>)}</div></section></section>}
  {activeTab==='Cadastros'&&catalogSection==='Pessoas'&&<section className="catalog-layout"><form className="panel catalog-form" onSubmit={savePerson}><div className="panel-title"><div><p className="eyebrow">{editingPerson?'EDITAR':'NOVA'} PESSOA</p><h2>{editingPerson?'Atualizar pessoa':'Adicionar pessoa'}</h2></div></div><label><span>Nome</span><input value={personName} onChange={e=>setPersonName(e.target.value)} placeholder="Nome da pessoa" required /></label><label><span>Percentual considerado</span><input type="number" min="0" max="100" value={personPercentage} onChange={e=>setPersonPercentage(Number(e.target.value))} /></label><label className="check catalog-check"><input type="checkbox" checked={personIncluded} onChange={e=>setPersonIncluded(e.target.checked)} /> Incluir nos cálculos do orçamento</label><div className="catalog-form-actions">{editingPerson&&<button type="button" className="attach" onClick={()=>{setEditingPerson('');setPersonName('');setPersonPercentage(100);setPersonIncluded(true)}}>Cancelar</button>}<button className="save" disabled={busy}>{editingPerson?'Salvar alterações':'Adicionar'}</button></div>{notice&&<p className="form-notice" role="status">{notice}</p>}</form><section className="panel catalog-panel"><div className="panel-title"><div><p className="eyebrow">DIVISÃO DOS GASTOS</p><h2>Pessoas</h2></div><span className="count-badge">{people.length}</span></div><div className="catalog-list">{people.map(item=><article key={item.id}><span className="catalog-icon person-icon">{item.name.charAt(0).toUpperCase()}</span><div><strong>{item.name}</strong><small>{item.include_in_calculation?`${item.calculation_percentage}% considerado`:'Fora do cálculo'}</small></div><div className="row-actions"><button type="button" onClick={()=>{setEditingPerson(item.id);setPersonName(item.name);setPersonPercentage(item.calculation_percentage);setPersonIncluded(item.include_in_calculation)}}>Editar</button><button type="button" className="danger" onClick={()=>removeCatalog('financial_people',item.id,item.name)}>Excluir</button></div></article>)}</div></section></section>}
  {activeTab==='Cadastros'&&catalogSection==='Origens'&&<section className="catalog-layout"><form className="panel catalog-form" onSubmit={saveSource}><div className="panel-title"><div><p className="eyebrow">{editingSource?'EDITAR':'NOVA'} ORIGEM</p><h2>{editingSource?'Atualizar origem':'Adicionar origem'}</h2></div></div><label><span>Nome</span><input value={sourceName} onChange={e=>setSourceName(e.target.value)} placeholder="Ex.: Cartão principal" required /></label><div className="compact-fields"><label><span>Dia do vencimento</span><input type="number" min="1" max="31" value={sourceDueDay} onChange={e=>setSourceDueDay(e.target.value)} placeholder="Opcional" /></label><label><span>Dia do fechamento</span><input type="number" min="1" max="31" value={sourceClosingDay} onChange={e=>setSourceClosingDay(e.target.value)} placeholder="Opcional" /></label></div><label><span>Últimos 4 dígitos</span><input inputMode="numeric" maxLength={4} value={sourceLastFour} onChange={e=>setSourceLastFour(e.target.value.replace(/\D/g,''))} placeholder="Somente para cartão" /></label><div className="catalog-form-actions">{editingSource&&<button type="button" className="attach" onClick={()=>{setEditingSource('');setSourceName('');setSourceDueDay('');setSourceClosingDay('');setSourceLastFour('')}}>Cancelar</button>}<button className="save" disabled={busy}>{editingSource?'Salvar alterações':'Adicionar'}</button></div>{notice&&<p className="form-notice" role="status">{notice}</p>}</form><section className="panel catalog-panel"><div className="panel-title"><div><p className="eyebrow">FORMAS DE PAGAMENTO</p><h2>Origens</h2></div><span className="count-badge">{origins.length}</span></div><div className="catalog-list">{origins.map(item=><article key={item.id}><span className="catalog-icon">◇</span><div><strong>{item.name}{item.card_last_four?` •••• ${item.card_last_four}`:''}</strong><small>{item.due_day?`Vence dia ${item.due_day}`:'Sem vencimento'}{item.closing_day?` · Fecha dia ${item.closing_day}`:''}</small></div><div className="row-actions"><button type="button" onClick={()=>{setEditingSource(item.id);setSourceName(item.name);setSourceDueDay(item.due_day?String(item.due_day):'');setSourceClosingDay(item.closing_day?String(item.closing_day):'');setSourceLastFour(item.card_last_four||'')}}>Editar</button><button type="button" className="danger" onClick={()=>removeCatalog('expense_sources',item.id,item.name)}>Excluir</button></div></article>)}</div></section></section>}
